<?php
	...
	include_once(BASE_DIR . '/lib/Equipment.class.php');
	global $auth;
	global $conf;
	...
	$res_id = filter_input(INPUT_POST, 'resid');
	if (is_null($res_id)) {
		$res_id = filter_input(INPUT_GET, 'resid');
	}
	...
	if ( !$auth->isLoggedIn() && ($read_only === null || $read_only == false) && $conf['app']['readOnlyDetails']) {
		$auth->printLoginMsg();
	} else {
		
		if ($is_blackout) {
			include_once(BASE_DIR . '/lib/Blackout.class.php');
			$res = new Blackout($res_id, $lab_id);
		} else {
			include_once(BASE_DIR . '/lib/Reservation.class.php');
			$res = new Reservation($res_id, false, $is_pending, $lab_id);
		}

		$t = new Template();
		
		if (strstr($_SERVER['HTTP_REFERER'], $_SERVER['PHP_SELF']) && !is_null($fn)) {
			$t->setTitle(translate("Processing $reservation_type_text"));
			$t->printHTMLHeader();
			$t->startMain();
	...
	function process_reservation(Reservation $res, $fn) {
		...
		$res_id         = filter_input(INPUT_POST, 'resid', FILTER_SANITIZE_STRING);
		if (is_null($res_id)) {
		...
		$mach_id        = filter_input(INPUT_POST, 'machid', FILTER_SANITIZE_STRING);
		...
		if (is_null($mach_id)) {
			$mach_id = $res->getResourceId();
		}
		$resource = new Equipment($mach_id);
		if (is_null($minRes)) {
		...
		if ($fn == RES_TYPE_ADD)
			$res->addReservation($mach_id, $invited_users, $user_id, $start_date, $end_date, $start_time, $end_time, $repeat, $minRes, $maxRes, $summary, $lab_id, $account_id);
		else if ($fn == RES_TYPE_MODIFY)
			$res->modifyReservation($user_id, $invited_users, $removed_users, $start_date, $end_date, $start_time, $end_time, !is_null($del), $minRes, $maxRes, !is_null($mod_recur), $account_id, str_replace("\n", "", $summary));
		else if ($fn == RES_TYPE_DELETE)
			$res->deleteReservation(!is_null($mod_recur));
		else if ($fn == RES_TYPE_APPROVE)
			$res->approveReservation(!is_null($mod_recur));
	...
	function present_reservation(Reservation $res) {
		...
		if ($res->getReservationId() === null) {
			$res->resource_id 	    = filter_input(INPUT_GET, 'machid', FILTER_SANITIZE_STRING);
			...
			$res->start_time         = filter_input(INPUT_GET, 'start_time', FILTER_SANITIZE_STRING);
			$res->end_time           = filter_input(INPUT_GET, 'end_time', FILTER_SANITIZE_STRING);
		}
		$res->setReservationType(filter_input(INPUT_GET, 'type', FILTER_SANITIZE_STRING));
		$res->printReservation();
	}
