                ...
                <td class="tableBorder">
                    <table width="100%" border="0" cellspacing="1" cellpadding="0">
                        <tr>
                            ...
                            <td class="tableTitle">
                                <div align="right">
									<?php $link->doLink('', '?', '', 'color: #FFFFFF', translate('Help') . ' - ' . translate('My Permissions')) ?>
                                </div>
                            </td>
                        </tr>
                    </table>
							...
							<?php
								...
								if (!$per){
									...
									for ($i = 0; is_array($per) && $i < count($per); $i++) {
										...
										echo "<span class='operational-status-" . strtolower($rs['operational_status']) . "'>" . translate($rs['operational_status']) . "</span>";
										echo "</td>\r\n";
										...
										// if the tool is operationally offline then no reservations allowed. Else, if reservation status is off and resource is not offline, user should contact staff.
										if ($rs['operational_status'] !== 'Offline') {
										    if ($rs['status'] === 'a') {
											    $link->doLink("javascript: reserve('".RES_TYPE_ADD."', '".$rs['machid']."','".time()."', '', '".$rs['lab_id']."', '0', '0', '');", 'Make Reservation', '', '','Make a Reservation');
										    } else if ($rs['status'] === 'u') {
											    echo translate('Contact staff to reserve');
										    }
										}
