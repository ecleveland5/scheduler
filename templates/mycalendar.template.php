...
function print_month_reservations($reservations, $datestamp, $fields = array('name'), $ownerParticipantImage = true, $is_private = false) {
	...
	echo "<table border=\"0\" tableLayout=\"fixed\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\"><tr><td class=\"tableBorder\">\n<table border=\"0\" width=\"100%\" cellspacing=\"1\" cellpadding=\"0\">\n";
	for ($day = $week_start; $day < $week_start + 7; $day++) {
		echo '<td class="labDate">' . $days_full[$day%7] . '</td>';
	}
	...
	for ($currentDay = -$firstWeekDay; $currentDay < $date_vars[3]; /* No change needed */ ) {
		...
		for ( /* Day already set */ ; $day < $week_start + 7; $day++) {
			...
			echo "<td class=\"$class\"><p align=\"right\"><a class=\"MyCalDateNumber\" href=\"rescalendar.php?date={$actualCurrentMonth}-$actualCurrentDay-{$actualCurrentYear}\">$actualCurrentDay</a></p>\n";
