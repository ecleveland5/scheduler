...
class Calendar {
                ...
                for ( /* Day already set */ ; $day < $this->weekstart + 7; $day++) {
                    ...
                    if (++$currentDay <= $this->daysInMonth) {
						if ($currentDay == $today['mday'] && $this->month == $today['mon'] && $this->year == $today['year'])
							...
							$class = 'dayBoxStyle';
                        echo "<td class=\"$class\"><a href=\"javascript: " . sprintf($this->javascript, $this->month, $currentDay, $this->year, intval($this->isPopup), $this->lab_id) . "\">" . ($currentDay) . "</a></td>";
                    }
					else {
	...
	*/
	function printJumpForm() {
		global $months_full;
		?>
         <form id="changeMonth" name="changeMonth" action="<?php echo $_SERVER['PHP_SELF'] . '?' . $this->lab_id ?>" method="post" style="margin-bottom: 0px;">
        ...
        ?>
				  ...
				  <input type="hidden" name="lab_id" value="<?= $this->lab_id?>" />
        		</td>
