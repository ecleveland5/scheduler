...
class Stats {
	...
	*/
	function init() {
		$this->numUsers = $this->db->get_quick_stats('user');
		...
		$users = $this->db->get_table_data('user', array ('user_id, first_name, last_name'), array ('last_name', 'first_name'));
		for ($i = 0; $i < count($users); $i++)
			$this->userids[$users[$i]['user_id']] = $users[$i]['last_name'] . ', ' . $users[$i]['first_name'];
	...
	*/
	function set_lab($lab_id) {
		if (empty($lab_id))
			$this->lab_id = $this->db->get_default_id();
		else
			$this->lab_id = $lab_id;
		
		return $this->db->check_lab_id($this->lab_id);
	}
	...
	*/
	...
	/**
	* Gets the list of labs
	* @param none
	* @return array of labs
	*/
	function get_lab_list() {
		return $this->db->get_lab_list();
	}
	
	/**
	* Prints out lab invalid error
	* @param none
	*/
	function print_lab_error() {
	...
	*/
	function parse() {
		...
		for ($i = 0; $i < count($res); $i++) {
			...
			$u[$res[$i]['user_id']]['all'] = (!isset($u[$res[$i]['user_id']]['all'])) ? 1 : $u[$res[$i]['user_id']]['all'] + 1;
			if ($this->active_user['num'] < $u[$res[$i]['user_id']]['all']) {	// Find most active user
				$this->active_user['num'] = $u[$res[$i]['user_id']]['all'];
				$this->active_user['id']  = $res[$i]['user_id'];
				$this->active_user['name'] = $this->userids[$res[$i]['user_id']];
			}
			// Group by user for each resource
			$u[$res[$i]['user_id']][$res[$i]['machid']] = (!isset($u[$res[$i]['user_id']][$res[$i]['machid']])) ? 1 : $u[$res[$i]['user_id']][$res[$i]['machid']] + 1;
