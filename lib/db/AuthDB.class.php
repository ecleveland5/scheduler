<?php
	...
	class AuthDB extends DBEngine {
		 ...
		 */
		function isSignedIn($user_id, $lab_id){
		 ...
		 */
		function logSignIn($user_id, $lab_id){
		...
		function isSignedInResource($equipment_id, $user_id=''){
		...
		function logEquipmentSignIn($user_id, $equipment_id, $frs){
		...
		function logEquipmentSignOut($useid, $equipment_id, $description, $notes, $problems){
		 ...
		 */
		function getUserList($type=NULL, $all=FALSE){
			$users = array();
		...
		function getUserPerms($userid){
			...
			$user = new UserDB;
			$perms = array();
			$perms = $user->getUserResourcePerms($userid);
			return $perms;
		}
		 ...
		 * @param 2-d array $users id, full name
		 */
		function getSignedInUserList($lab_id=1, $order=''){
			$users = array();
		...
		function getSignedInUser($signid=''){
		...
		public function getUserLabPermissions($user_id, $lab_id = null) {
			$return = array();
		...
		public function getUserSystemPermissions($user_id, $system_resource_id) {
			$return = array();
		...
		public function createUserSystemPermissions($user_id, $system_resource_id, array $permissions) {
		...
		public function getSessionHashByUserId($user_id) {
			$sql = "SELECT session_hash FROM `sessions` WHERE user_id = ?";
			$q = $this->db->prepare($sql);
