...
class DB_pgsql extends DB_common
{
    ...
    function _pgFieldFlags($resource, $num_field, $table_name)
    {
        ...
        $result = @pg_exec($this->connection, "SELECT f.attnotnull, f.atthasdef
                                ...
                                WHERE tab.relast_name = typ.typname
                                AND typ.typrelid = f.attrelid
                                AND f.attname = '$field_name'
                                AND tab.relast_name = '$table_name'");
        if (@pg_numrows($result) > 0) {
            ...
            if ($row[1] == 't') {
                $result = @pg_exec($this->connection, "SELECT a.adsrc
                                    ...
                                    WHERE tab.relast_name = typ.typname AND typ.typrelid = f.attrelid
                                    AND f.attrelid = a.adrelid AND f.attname = '$field_name'
                                    AND tab.relast_name = '$table_name' AND f.attnum = a.adnum");
                $row = @pg_fetch_row($result, 0);
    ...
    function getSpecialQuery($type)
    {
        switch ($type) {
            case 'tables':
                return "SELECT c.relast_name as \"Name\"
                        FROM pg_class c, pg_user u
                        WHERE c.relowner = u.usesysid AND c.relkind = 'r'
                        AND not exists (select 1 from pg_views where viewname = c.relast_name)
                        AND c.relast_name !~ '^(pg_|sql_)'
                        UNION
                        SELECT c.relast_name as \"Name\"
                        FROM pg_class c
                        WHERE c.relkind = 'r'
                        AND not exists (select 1 from pg_views where viewname = c.relast_name)
                        AND not exists (select 1 from pg_user where usesysid = c.relowner)
                        AND c.relast_name !~ '^pg_'";
            case 'views':
